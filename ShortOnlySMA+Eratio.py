...
#This is an outdated edge ratio calculator and strategy tester - see for updated version.
...
Asset1['RollingMax'] = Asset1['High'].rolling(window=atrwindow, center=False).max()
Asset1['RollingMin'] = Asset1['Low'].rolling(window=atrwindow, center=False).min()
...
Asset1['Regime'] = np.where(Asset1['High'] > (Asset1['SMA']*1.25), -1, 0)
ranger = range(0, 20)
for r in ranger:
    Asset1['Regime'] = np.where(Asset1['Regime'].shift(1) == -1, -1, Asset1['Regime'])
#Asset1['Regime'] = np.where(Asset1['Adj Close'] > (Asset1['RollingMax']), 1, Asset1['Regime'])
#for r in ranger:
#    Asset1['Regime'] = np.where(Asset1['Regime'].shift(1) == 1, 1, Asset1['Regime'])
