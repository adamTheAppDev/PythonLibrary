...
RiskPerTrade = .005
DollarRisk =  0 #Equity * RiskPerTrade
...
Asset1['RollingMax'] = Asset1['High'].rolling(window=ROCWindow, center=False).max()
Asset1['RollingMin'] = Asset1['Low'].rolling(window=ROCWindow, center=False).min()
#Signal = Price </> min/max
#if price is greater than the max go long
Asset1['LongSignal'] = 0 #np.where(Asset1['High'] > Asset1['RollingMax'].shift(1), 1, 0)
#if price is less than the min go short
Asset1['ShortSignal'] = np.where(Asset1['RateOfChange'] > .2, 1, 0)
...
#while Counter < 1: #Use this instead of the while loop to go a certain number of trades into the iteration; to examine TradeSubset
...
while sum(abs(TradeSubset['Signal'])) != 0:
    ...
    try:
        IndexOfExit = TradeSubset.index[HoldWindow - 1]
    except IndexError:
        IndexOfExit = TradeSubset.index[len(TradeSubset) - 1]
    ...
    numshares = 0 #(DollarRisk)/((TradeATR * numATR))//1
    #1 = long; -1 = short
    TradeDirection = TradeSubset['Signal'][0]
    #For long trades
    if TradeDirection == 1:
        #Establish non-gap entry price based on previous period rolling max
        EntryPrice = Asset1['Adj Close'].loc[Asset1.index == TradeSubset.index[0]][0]
...
#        try:
        ExitPrice = TradeSubset['Adj Close'].loc[TradeSubset.index == IndexOfExit][0]
...
#            GapExit = 1
        ...
        TradePercentReturn = (ExitPrice - EntryPrice) / EntryPrice
        TradeDollarReturn = 0 #(ExitPrice - EntryPrice) * numshares
...
#            ExitPrice = TradeSubset['Open'].loc[IndexOfExit]
            #Record gap exit            
#            GapExit = 1
        TradePercentReturn = (EntryPrice - ExitPrice) / EntryPrice
        TradeDollarReturn = 0 #(EntryPrice - ExitPrice) * numshares
    #If there is no exit signal, the trade is still open..    
#    if math.isnan(ExitPrice) == True:
#            break
        
    LengthOfTrade = HoldWindow
...
#    SubIndexOfExit = TradeSubset['SubIndex'].loc[IndexOfExit]
    ...
    RMultiple = 0 #TradeDollarReturn / DollarRisk
...
Trades = Trades.rename(index={0: "TradeDirection", 1: "IndexOfEntry", 2: "LengthOfTrade",
    3: "IndexOfExit", 4:"EntryPrice", 5: "ExitPrice", 6: "numshares", 7: "TradeATR",
    8: "TradePercentReturn", 9: "TradeDollarReturn", 10: "DollarRisk",
    11: "RMultiple"})
#    
...
for d in Trades:
    Asset1['StrategyPercentReturns'].loc[(Asset1.index == Trades[d]['IndexOfExit'])] = 1 + Trades[d]['TradePercentReturn']
