...
#On the first signal record entry, create stop regime, record  exit, record 
...
while sum(abs(TradeSubset['Signal'])) != 0:
    ...
    if TradeDirection == 1:
        ...
        #Use the cumulative maximum; thus preventing the stop from moving down.
...
#Reset gap indicators
    ...
    if TradeDirection == -1:
        ...
        TradeSubset['StopPriceUnitOne'] = TradeSubset['StopPriceUnitOne'].fillna(TradeSubset['RollingShortStop'])
        #Use the cumulative minimum; thus preventing the stop from moving down.
        TradeSubset['StopPriceUnitOne'] = TradeSubset['StopPriceUnitOne'].cummin()
