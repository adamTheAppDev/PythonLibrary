...
a = 10 #number of days for moving average window
b = 40 #numer of days for moving average window
multiplierA = (2/(a+1))
multiplierB = (2/(b+1))
Range = range(0, len(s['Adj Close']))
s['LogReturns'] = np.log(s['Adj Close']/s['Adj Close'].shift(1))
s['SmallSMA'] = s['Adj Close'].rolling(window=a, center=False).mean()
s['LargeSMA'] = s['Adj Close'].rolling(window=b, center=False).mean()
