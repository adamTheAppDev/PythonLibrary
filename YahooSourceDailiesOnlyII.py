...
#Prior to running this, run EntityCreation or FolderFramework and place
#NASDAQData.csv and UniverseList.csv in appropriate folders or assign to variables 
#Yahoo sourcing for dailies only 
...
#Define function to save CSV daily frequency time series to folder
...
NumIssues = range(0,len(UniverseList['Ticker']))
#For every issue
for n in NumIssues:
    try: 
        #Take the symbol from the UniverseList
        Ticker = str(UniverseList['Ticker'][n])
...
print('All daily data is in preprocess storage as CSV; ready for processing.')

#Processed data will be stored as FREQ-TICKER-YYYYMMDD
...
NumSavedCSVs = range(0,len(TimeSeries))

for CSVnum in NumSavedCSVs:
    try:
        temp = read_csv(DL + '\\DataSources\\YahooSource\\TimeSeriesData\\' +
                         (TimeSeries[CSVnum]), sep = ',')
        print('Basic TS processing for ' + TimeSeries[CSVnum] +'.')
        ...
        temp['FrequencyPrefix'] = TimeSeries[CSVnum][:3]
        temp['Ticker'] = TimeSeries[CSVnum][4:-4]
...
#Populate set - DAY - with custom database modifications for TA data.
#Start with Yahoo dailies TA mod
print('Processing for dailies.')
#Get list to process
ProcessedDailies = os.listdir(DL + '\\DataSources\\YahooSource\\ProcessedData\\DAY\\')
for p in ProcessedDailies:
    #Access data
    temp = pd.read_pickle(DL + '\\DataSources\\YahooSource\\ProcessedData\\' + 
                                        'DAY\\' + p + '\\' + p)
