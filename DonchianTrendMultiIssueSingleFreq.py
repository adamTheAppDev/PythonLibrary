...
#ModelOutput destination
ModelOutput = pd.DataFrame()
for dt in DailiesTickers[:]:
    print('Grabbing data for ' + dt)
    Asset1 = YahooSourceDailyGrabber(dt)
...
#    Asset1 = Asset1[:1500] #In
    ...
    while sum(abs(TradeSubset['Signal'])) != 0:
        ...
        if TradeDirection == 1:
            ...
            for ll in LongExitIndexList:
                TradeSubset.set_value(ll, 'LongExit', 1)
        if TradeDirection == -1:        
            ...
            for ss in ShortExitIndexList:
                TradeSubset.set_value(ss, 'ShortExit', 1)
        ...
        if TradeDirection == 1:
            ...
            for lll in GapLongExitIndexList:
                TradeSubset.set_value(lll, 'GapLongExit', 1)
    ...
    #Compare that to the df[].loc[()] = way of assignment 4 lines after that. Which is best?
            ...
            for sss in GapShortExitIndexList:
                TradeSubset.set_value(sss, 'GapShortExit', 1)
    ...
    #Descriptive Stats
    dailyreturn = Asset1['StrategyPercentReturns'].mean()
    dailyvol = Asset1['StrategyPercentReturns'].std()
...
#    Asset1['DollarPL'].plot()
    ...
    #Make a DataFrame that stores model output.
    SMO = [sharpe, MaxDD, Expectancy, WinRate, LossRate, sum(Asset1['DoubleDay'])]
    SingleModelOutput = pd.Series(SMO)
    #SingleModelOutput = SingleModelOutput.append(SingleModelOutput)
    ModelOutput[dt] = SingleModelOutput
...
print('Best Expectancies')
print(Expectancies.tail(10))
